...
const fromEntries =
  Object.fromEntries || ((entries) => entries.reduce((obj, [k, v]) => Object.assign(obj, { [k]: v }), {}))
...
class Config {
  constructor (options) {
    ...
    this.serviceMapping = DD_SERVICE_MAPPING.length
      ? fromEntries(DD_SERVICE_MAPPING.split(',').map((x) => x.trim().split(':')))
      : {}
